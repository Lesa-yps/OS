...
void daemonize(const char *cmd)
{
    ...
    * почему у демонов не должно быть управляющего терминала?
    ...
    */
    ...
    sa.sa_flags = 0;
    if (sigaction(SIGHUP, &sa, NULL) < 0) /* в результате будет игнорироваться сигнал SIGHUP, чтоб терминал не завершился аварийно, тк лишили управляющего терминала */
    ...
    * тк в файловой системе файловые системы монтируются, юникс поддерживвает много ФС, но только смонтированная файловая система доступна пользователю и предоставляет доступ к файлам и каталогам
    */
    if (chdir("/") < 0)
    ...
    * каждый процесс имеет собственную таблицу открытых файлов (размер таблицы = 512, тк есть в ядре функция, которая ищет в таблице первый свободный дескриптор, если все дескрипторы заняты система переходит на новую таблицу из 512 дескрипторов -> итого 1024, все 101/24 заняты -> ошибка, что нельзя открыть файл)
    */
    ...
    for (i = 0; i < rl.rlim_max; i++) /* закрываются все открытые файлы, начиная с 0 (автоматически в таблицу добавляются stdin, stdout and stderr)*/
     ...
     /*
    * -- Присоединить файловые дескрипторы 0, 1 и 2 к /dev/null. (чтоб, если вдруг у какого-то деомна возникнет фантазия что-то куда-то записать, чтоб это ушло в нулл)
    */
